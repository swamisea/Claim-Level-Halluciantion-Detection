    ...
    def get_score(self,
        ...
        for topic, generation, facts in zip(topics, generations, atomic_facts):
            if facts is None:
                ...
                # Filter out None (insufficient context) atoms for the binary score
                binary = [d["is_supported"] for d in decision if d["is_supported"] is not None]
                score = np.mean(binary) if binary else 0.0
        ...
        for atom in atomic_facts:
            ...
            if self.lm:
                ...
                if not definition[-1] in string.punctuation:
                    definition += "."
                prompt = "{}\n\nInput: {} True, False, or Neither (if the context does not contain enough information)?\nOutput:".format(definition.strip(), atom.strip())
                ...
                if type(output[1])==np.ndarray:
                    ...
                    # 3-way parse: True / False / Neither
                    generated_answer = output[0].lower().strip()
                    nei_keywords = ["neither", "not enough", "insufficient", "cannot determine",
                                    "unclear", "not mentioned", "no information", "does not"]
                    if any(kw in generated_answer for kw in nei_keywords):
                        is_supported = None   # NEI atom — insufficient context
            ...
            # is_supported: True = supported, False = contradicted, None = insufficient context
