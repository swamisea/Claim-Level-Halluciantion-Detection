...
class AtomicFactGenerator(object):
    def __init__(self, key_path, demon_dir, gpt3_cache_file=None):
        ...
        with open(self.demon_path, 'r') as f:
            demons_raw = json.load(f)
        # Handle list format [[topic, text, facts], ...] as well as dict format {text: facts}
        if isinstance(demons_raw, list):
            self.demons = {item[1]: item[2] for item in demons_raw}
        else:
            self.demons = demons_raw
    ...
    def get_init_atomic_facts_from_sentence(self, sentences, cost_estimate=None):
        ...
        n = min(7 if is_bio else 8, len(demons))
